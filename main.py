...
vector_store_id = "vs_L0XWhSKPr83LMAh7rpLR2Wqc"
...
# ==== Function defenitions etc =====
def upload_to_openai(file_path):
    with open(file_path, "rb") as file:
        response = client.files.create(file=(os.path.basename(file_path),file), purpose="assistants")
    return response.id

# ====Sidebar - where users can uppload files
...
if st.session_state.file_id_list:
    st.sidebar.write("Upload File IDs:")
    for file_id in st.session_state.file_id_list:
        st.siderbar.write(file_id)
        # Associate each file id with the current assistant
        assistant_file = client.beta.assistants.update(assistant_id=assis_id,
                                                       tool_resources={"file_search":{"vector_store_ids":[vector_store_id]}}
                                                       )
        # assisntant_file = client.beta.assistants.files.create(
...
if st.session_state.start_chat:
    ...
    if prompt := st.chat_input("What's new"):
        ...
        # Create a run with addidtional instructions
